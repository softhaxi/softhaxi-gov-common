...
public interface NotificationRepository extends JpaRepository<Notification, UUID> {
    ...
    //         "LEFT JOIN NotificationStatus B ON B.message.id=A.id AND (B.user IS NULL AND B.delivered IS NULL) OR (B.user = ?1 AND B.delivered = false) " + 
    ...
    //     "WHERE A.id IN " + 
    //     "(SELECT A.id FROM Notification " +
    //         "LEFT JOIN NotificationStatus B ON B.message.id=A.id AND (B.user IS NULL OR B.user = ?1) " + 
    //         "WHERE A.level='PUBLIC')" +
    //     "OR A.id IN " + 
    //     "(SELECT A.id FROM Notification " +
    //         "JOIN NotificationStatus B ON B.message.id=A.id " + 
